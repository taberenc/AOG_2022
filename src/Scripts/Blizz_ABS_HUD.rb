...
class Hotkey_Assignment
  ...
  def initialize(viewport = nil)
    ...
    self.x, self.y, self.z = 4, 40, 1100
...
class Window_Skill_Hotkey < Window_Skill
  ...
  def initialize(actor)
    super
    @column_max = 1
    self.width, self.height = 260, 400
  ...
  def draw_item(i)
    if @data[i] == nil
      ...
      self.contents.blt(4, 4+i*32, bitmap, Rect.new(0, 0, 24, 48), opacity)
      text = @data[i].name
...
class Window_Item_Hotkey < Window_Item
  ...
  def initialize
    super
    @column_max = 1
    self.width, self.height = 288, 400
